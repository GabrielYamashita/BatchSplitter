...
def main():
    ...
    assert list(df_output.columns) == ["nome", "WHATSAPP"]
    assert report["renamed_columns"] == {"telefone": "WHATSAPP"}
    assert report["mapped_unchanged_columns"] == []
    assert report["kept_unmapped_columns"] == ["nome"]

    assert df_output.loc[0, "nome"] == "Gabriel"
    assert df_output.loc[0, "WHATSAPP"] == "011999999999"

    print("Output Builder Template Override OK\n")
